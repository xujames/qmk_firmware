...
static uint8_t randomize_click_timing(uint8_t click_period)
{
    return click_period + rand() % 15;    // improve this later...
}
...
static uint32_t turbo_click_callback(uint32_t trigger_time, void* cb_arg) {
  ...
  return randomize_click_timing(CLICK_PERIOD_MS) / 2;  // Execute callback again in half a period.
}
